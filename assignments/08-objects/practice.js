...
var isUser = function (a) {
	...
	var s=keys.forEach(function(words){
		if((words=="name")||(words=="screen_name"))
		{
			c++;
		}
		});
...
//
var userToDiv = function (a) {
	if(isUser(a)==true)
	{
		return "<div>"+"<h1>"+a.name+"</h1>"+"<h2>"+a.screen_name+"</h2"+"</div>";
	}
	else
		return false
};
...
//
var userWithTweetsToDiv = function (a) {
	string=""
	string="<div><h1>"+a.name+"</h1><h2>"+a.screen_name+"</h2>";
	var tweets=a.tweets;
	for(var i=0;i<tweets.length;i++)
	{
	string=string+"<li>"+tweets[i]+"</li>";
	}
